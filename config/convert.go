...
func etcdContents(etcd types.Etcd) string {
	...
	// Add the tag
	vars = append(vars, fmt.Sprintf("ETCD_IMAGE_TAG=%s", etcd.Version))
...
func flannelContents(flannel types.Flannel) string {
	...
	// Add the tag
	vars = append(vars, fmt.Sprintf("FLANNEL_IMAGE_TAG=%s", flannel.Version))
	return serviceContentsFromEnvVars(vars)
}
